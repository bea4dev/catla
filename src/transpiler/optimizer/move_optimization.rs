...
#[must_use]
struct LastUseEnvironment<'allocator> {
    last_use_position_map:
        HashMap<EntityID, Vec<EntityID, &'allocator Bump>, DefaultHashBuilder, &'allocator Bump>,
    allocator: &'allocator Bump,
}

impl<'allocator> LastUseEnvironment<'allocator> {
    fn new(allocator: &'allocator Bump) -> Self {
        Self {
            last_use_position_map: HashMap::new_in(allocator),
            allocator,
        }
    }

    fn marge_with(&mut self, other: Self) {
        for (other_key, other_values) in other.last_use_position_map.into_iter() {
            if let Some(values) = self.last_use_position_map.get_mut(&other_key) {
                values.extend(other_values);
            } else {
                self.last_use_position_map.insert(other_key, other_values);
            }
        }
    }

    fn update_with(&mut self, other: Self) {
        self.last_use_position_map
            .extend(other.last_use_position_map);
    }
...
fn collect_factor<'allocator>(
    ...
    if let Ok(primary) = &ast.primary {
        ...
        LastUseEnvironment::new(allocator)
    }
}
