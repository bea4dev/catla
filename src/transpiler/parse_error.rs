...
pub fn collect_parse_error_program(ast: Program, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    ...
    for statement in ast.statements.iter() {
        ...
        match statement {
            ...
            StatementAST::Import(import) => {
                collect_error_tokens(&import.elements.error_tokens, Expected::ImportElements, 0007, errors, context);
                collect_parse_error_only_parse_result_error(&import.elements.brace_right, Expected::BraceRight, 0007, errors, context);
            },
            ...
            StatementAST::VariableDefine(variable_define) => {
                collect_parse_error_only_parse_result_error(&variable_define.name, Expected::VariableName, 0008, errors, context);
                if let Some(expression) = &variable_define.expression {
                    collect_parse_error_with_parse_result(expression, collect_parse_error_expression, Expected::Expression, 0008, errors, warnings, context);
                }
            },
            StatementAST::FunctionDefine(function_define) => {
                ...
                collect_parse_error_only_parse_result_error(&arguments.paren_left, Expected::ParenthesisLeft, 0009, errors, context);
                for argument in arguments.arguments.iter() {
                    collect_parse_error_type_tag(&argument.type_tag, errors, warnings, context);
                }
                
                collect_error_tokens(&arguments.error_tokens, Expected::FunctionArgument, 0009, errors, context);

                collect_parse_error_only_parse_result_error(&arguments.paren_right, Expected::ParenthesisRight, 0009, errors, context);

                if let Some(type_tag) = &function_define.type_tag {
                    collect_parse_error_type_tag(type_tag, errors, warnings, context);
                }

                collect_parse_error_with_recovered(&function_define.block, collect_parse_error_block, Expected::Block, 0009, errors, warnings, context);
            },
            StatementAST::DataStructDefine(data_struct_define) => {
                ...
                collect_parse_error_with_recovered(&data_struct_define.block, collect_parse_error_block, Expected::Block, 0010, errors, warnings, context);
            },
            StatementAST::DropStatement(drop_statement) => {
                collect_parse_error_with_parse_result(&drop_statement.expression, collect_parse_error_expression, Expected::Expression, 0011, errors, warnings, context);
            },
...
fn collect_parse_error_block(ast: &Block, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    ...
    collect_parse_error_only_parse_result_error(&ast.brace_right, Expected::BraceRight, 0012, errors, context);
}
    ...
    match ast {
        ExpressionEnum::OrExpression(or_expression) => {
            ...
            for expression in or_expression.right_exprs.iter() {
                collect_parse_error_with_parse_result(&expression.1, collect_parse_error_and_expression, Expected::Expression, 0013, errors, warnings, context);
            }
        },
        ...
        ExpressionEnum::Closure(closure) => {
            ...
            collect_parse_error_only_parse_result_error(&arguments.vertical_bar_right, Expected::VerticalBarRight, 0014, errors, context);
...
fn collect_parse_error_expression_or_block(ast: &Either<Expression, Block>, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    match ast {
...
fn collect_parse_error_and_expression(ast: &AndExpression, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    collect_parse_error_eqne_expression(&ast.left_expr, errors, warnings, context);
    for expression in ast.right_exprs.iter() {
        collect_parse_error_with_parse_result(&expression.1, collect_parse_error_eqne_expression, Expected::Expression, 0013, errors, warnings, context);
    }
}

fn collect_parse_error_eqne_expression(ast: &EQNEExpression, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    collect_parse_error_compare_expression(&ast.left_expr, errors, warnings, context);
    for expression in ast.right_exprs.iter() {
        collect_parse_error_with_parse_result(&expression.1, collect_parse_error_compare_expression, Expected::Expression, 0013, errors, warnings, context);
    }
}

fn collect_parse_error_compare_expression(ast: &CompareExpression, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    collect_parse_error_add_or_sub_expression(&ast.left_expr, errors, warnings, context);
    for expression in ast.right_exprs.iter() {
        collect_parse_error_with_parse_result(&expression.1, collect_parse_error_add_or_sub_expression, Expected::Expression, 0013, errors, warnings, context);
    }
}

fn collect_parse_error_add_or_sub_expression(ast: &AddOrSubExpression, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    collect_parse_error_mul_or_div_expression(&ast.left_expr, errors, warnings, context);
    for expression in ast.right_exprs.iter() {
        collect_parse_error_with_parse_result(&expression.1, collect_parse_error_mul_or_div_expression, Expected::Expression, 0013, errors, warnings, context);
    }
}

fn collect_parse_error_mul_or_div_expression(ast: &MulOrDivExpression, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    collect_parse_error_factor(&ast.left_expr, errors, warnings, context);
    for expression in ast.right_exprs.iter() {
        collect_parse_error_with_parse_result(&expression.1, collect_parse_error_factor, Expected::Expression, 0013, errors, warnings, context);
    }
}

fn collect_parse_error_factor(ast: &Factor, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    collect_parse_error_with_parse_result(&ast.primary, collect_parse_error_primary, Expected::Expression, 0013, errors, warnings, context);
}

fn collect_parse_error_primary(ast: &Primary, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
...
fn collect_parse_error_primary_left(ast: &PrimaryLeft, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    match &ast.first_expr {
        PrimaryLeftExpr::Simple(simple_primary) => {
            if let SimplePrimary::Expression { expression, error_tokens } = &simple_primary.0 {
                collect_parse_error_with_parse_result(expression, collect_parse_error_expression, Expected::Expression, 0013, errors, warnings, context);
        ...
        PrimaryLeftExpr::NewExpression(new_expression) => {
            ...
            collect_parse_error_with_parse_result(&new_expression.function_call, collect_parse_error_function_call, Expected::FunctionCall, 0016, errors, warnings, context);
        },
        ...
        PrimaryLeftExpr::LoopExpression(loop_expression) => {
            collect_parse_error_with_parse_result(&loop_expression.block, collect_parse_error_block, Expected::Block, 0018, errors, warnings, context)
        }
    }
}

fn collect_parse_error_function_call(ast: &FunctionCall, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    if let Some(generics) = &ast.generics {
        collect_parse_error_with_parse_result(generics, collect_parse_error_generics, Expected::Generics, 0015, errors, warnings, context);
    }
...
fn collect_parse_error_if_statement(ast: &IfStatement, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    collect_parse_error_with_parse_result(&ast.condition, collect_parse_error_expression, Expected::Expression, 0017, errors, warnings, context);
    collect_parse_error_with_recovered(&ast.block, collect_parse_error_block, Expected::Block, 0017, errors, warnings, context);
}

fn collect_parse_error_if_statement_or_block(ast: &Either<IfStatement, Block>, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    match ast {
...
fn collect_parse_error_primary_right(ast: &PrimaryRight, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    ...
    if let Some(mapping_operator) = &ast.mapping_operator {
        ...
        collect_parse_error_with_recovered(block, collect_parse_error_block, Expected::Block, 0019, errors, warnings, context)
    }
}

fn collect_parse_error_type_tag(ast: &TypeTag, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    collect_parse_error_with_parse_result(&ast.type_info, collect_parse_error_type_info, Expected::TypeInfo, 0020, errors, warnings, context);
}

fn collect_parse_error_type_info(ast: &TypeInfo, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
    if let Some(generics) = &ast.generics {
        collect_parse_error_generics(generics, errors, warnings, context);
    }
...
fn collect_parse_error_generics(ast: &Generics, errors: &mut Vec<TranspileError>, warnings: &mut Vec<TranspileWarning>, context: &TranspileModuleContext) {
