...
impl LifetimeEvaluator {
    ...
    pub async fn eval(&self, context: &TranspileContext) {
        ...
        loop {
            let futures = (0..num_threads).map(|_| self.eval_lifetime_source(context));
    ...
    async fn eval_lifetime_source(&self, context: &TranspileContext) -> (Vec<Arc<String>>, bool) {
        ...
        loop {
...

            {
                ...
                for (entity_id, lifetime_source) in lifetime_sources {
                    ...
                    if let Some(origin_define) = context
                    ...
                    for function_call in lifetime_source.instance.function_calls.iter() {
                        ...
                        let define = match context.function_equals_info.resolve(&define) {
                    ...
                    if let Some(origin_function) = context
                        .function_equals_info
                        .resolve(&(module_name.clone(), *entity_id))
                    {
