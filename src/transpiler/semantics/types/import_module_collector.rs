...
use catla_parser::parser::{
    ...
    ExpressionEnum, Factor, FunctionCall, Generics, GenericsDefine, Import, MappingOperator,
    MappingOperatorKind, MulOrDivExpression, NewExpression, Primary, PrimaryLeft, PrimaryLeftExpr,
    PrimaryRight, PrimarySeparatorKind, Program, SimplePrimary, Spanned, StatementAST,
    TupleTypeInfo, TypeAttributeEnum, TypeInfo, TypeTag,
};
use either::Either;
            ...
            StatementAST::UserTypeDefine(data_struct_define) => {
                ...
                if let Some(super_type_info) = &data_struct_define.super_type_info {
                    for type_info in super_type_info.type_infos.iter() {
                        collect_import_module_type_info(
                            type_info,
                            import_element_map,
                            name_resolved_map,
                            errors,
...
fn collect_import_module_import(
    ...
    if size == 0 {
        ...
        if context
            ...
            .exists_source_code_or_package(&module_name)
        {
    ...
    for element in ast.elements.elements.iter() {
        ...
        let module_name = if context
            .source_code_provider
            .exists_source_code_or_package(&module_name2)
        {
            module_name2
        } else if context
            .source_code_provider
            .exists_source_code_or_package(&module_name1)
        {
            module_name1
        } else {
                ...
                if let SimplePrimary::Identifier(literal) = &simple.0 {
                    ...
                    for i in 0..ast.chain.len().checked_sub(1).unwrap_or_default() {
                        ...
                        if let Some(expr) = &right_primary.second_expr {
                            ...
                            {
                                return Some((
                                    module_name[0..(module_name.len() - expr.0.value.len() - 2)]
                                        .to_string(),
                                    count,
                                ));
                            }
        ...
        PrimaryLeftExpr::Simple(simple) => {
            match &simple.0 {
                SimplePrimary::Expressions {
                    ...
                    for expression in expression.iter() {
                        collect_import_module_expression(
                            &expression,
                            import_element_map,
                            name_resolved_map,
...
pub(crate) fn get_module_name_from_new_expression(
    ...
    let mut module_name =
        if let Some(resolved) = name_resolved_map.get(&EntityID::from(&new_expression.path[0])) {
            let import_entity_id = resolved.define_info.entity_id;
            import_element_map
                .get(&import_entity_id)
                .unwrap()
...
fn collect_import_module_tuple_type_info(
    ast: &TupleTypeInfo,
    import_element_map: &mut FxHashMap<EntityID, Spanned<String>>,
    name_resolved_map: &FxHashMap<EntityID, FoundDefineInfo>,
    errors: &mut Vec<TranspileError>,
...
fn collect_import_module_base_type_info(
    ...
    if ast.path.len() > 1 {
        let first_module_name =
            if let Some(resolved) = name_resolved_map.get(&EntityID::from(&ast.path[0])) {
                let import_entity_id = resolved.define_info.entity_id;
                import_element_map
                    .get(&import_entity_id)
                    .cloned()
