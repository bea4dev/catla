    ...
    for statement in ast.statements.iter() {
        ...
        match statement {
            StatementAST::Assignment(assignment) => {
                collect_import_module_expression(assignment.left_expr, import_element_map, errors,warnings, context);
                if let Ok(right_expr) = &assignment.right_expr {
                    collect_import_module_expression(&right_expr, import_element_map, errors,warnings, context);
                }
            },
            StatementAST::Exchange(exchange) => {
                collect_import_module_expression(exchange.left_expr, import_element_map, errors,warnings, context);
            ...
            StatementAST::Import(import) => {
                collect_import_module_import(import, import_element_map, errors,warnings, context);
            },
            _ => {}
        }
    }
...
fn collect_import_module_import(
    ...
    for element in ast.elements.elements.iter() {
        ...
        import_element_map.insert(EntityID::from(element), module_name2.clone());
...
fn collect_import_module_expression(
    ...
    match ast {
        ExpressionEnum::OrExpression(or_expression) => {
            collect_import_module_and_expression(&or_expression.left_expr, import_element_map, errors,warnings, context);
            for right_expr in or_expression.right_exprs.iter() {
                if let Ok(right_expr) = &right_expr.1 {
                    collect_import_module_and_expression(right_expr, import_element_map, errors,warnings, context);
                }
            }
        },
...
fn collect_import_module_and_expression(
    ...
    collect_import_module_eqne_expression(&ast.left_expr, import_element_map, errors,warnings, context);
    for right_expr in ast.right_exprs.iter() {
        if let Ok(right_expr) = &right_expr.1 {
            collect_import_module_eqne_expression(right_expr, import_element_map, errors,warnings, context);
        }
    }
}
...
fn collect_import_module_eqne_expression(
    ...
    collect_import_module_compare_expression(&ast.left_expr, import_element_map, errors,warnings, context);
    for right_expr in ast.right_exprs.iter() {
        if let Ok(right_expr) = &right_expr.1 {
            collect_import_module_compare_expression(right_expr, import_element_map, errors,warnings, context);
        }
    }
}
...
fn collect_import_module_compare_expression(
    ...
    collect_import_module_add_or_sub_expression(&ast.left_expr, import_element_map, errors,warnings, context);
    for right_expr in ast.right_exprs.iter() {
        if let Ok(right_expr) = &right_expr.1 {
            collect_import_module_add_or_sub_expression(right_expr, import_element_map, errors,warnings, context);
        }
    }
}
...
fn collect_import_module_add_or_sub_expression(
    ...
    collect_import_module_mul_or_div_expression(&ast.left_expr, import_element_map, errors,warnings, context);
    for right_expr in ast.right_exprs.iter() {
        if let Ok(right_expr) = &right_expr.1 {
            collect_import_module_mul_or_div_expression(right_expr, import_element_map, errors,warnings, context);
        }
    }
}
...
fn collect_import_module_mul_or_div_expression(
    ...
    collect_import_module_factor(&ast.left_expr, import_element_map, errors,warnings, context);
    for right_expr in ast.right_exprs.iter() {
        if let Ok(right_expr) = &right_expr.1 {
            collect_import_module_factor(right_expr, import_element_map, errors,warnings, context);
        }
    }
}
...
fn collect_import_module_factor(
    ...
    if let Ok(primary) = &ast.primary {
        collect_import_module_primary(primary, import_element_map, errors,warnings, context);
    }
}
...
fn collect_import_module_primary_left(
    ...
    match &ast.first_expr {
        PrimaryLeftExpr::Simple(simple) => {
            ...
            if let Some(function_call) = &simple.1 {
                collect_import_module_function_call(function_call, import_element_map, errors,warnings, context);
            }
        },
        ...
        PrimaryLeftExpr::IfExpression(if_expression) => {
            ...
            if let Ok(condition) = &first_statement.condition {
                collect_import_module_expression(&condition, import_element_map, errors,warnings, context);
            }
    ...
    if let Some(mapping_operator) = &ast.mapping_operator {
        collect_import_module_mapping_operator(mapping_operator, import_element_map, errors,warnings, context);
    }
}
